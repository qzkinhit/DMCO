...
def refit_best_sklearn_model(automl,
    ...
    if isinstance(base_est, Pipeline):
        clf = base_est.named_steps.get("classifier", None)
        if isinstance(clf, AutoSklearnChoice):
            final_est = clf.choice
        else:
            final_est = clf
    else:
        final_est = base_est
...
class CostAwareMAB:
    ...
    def __init__(self,
        ...
        self.mu = {'clean': 0.6, 'AutoML': 0.4}  # 折扣和，对应折扣均值
...
def sample_base_grad(X, y, fraction, X_train_grad_norm, iter):
    if X_train_grad_norm is not None:
        selected_samples = np.argsort(-X_train_grad_norm)[
                           iter * int(fraction * len(X)):(iter + 1) * int(fraction * len(X))]
        return selected_samples, X_train_grad_norm
    else:
        # todo
        model_svc = SVC()
        model_svc.fit(X, y)
        ...
        # todo : duo yang xing
        return selected_samples, g_norm
...
def pick_families_from_leaderboard(automl, max_fams=3):
    try:
        ...
        return None
    col = "pipeline" if "pipeline" in lb.columns else 'type'
    # todo
    vocab = [
        'libsvm_svr', 'mlp', 'sgd'
    ]
...
def automl(X, y, families, r,
    ...
    #     # include={"regressor": ['libsvm_svr', 'mlp', 'sgd']},  # 先锁死一个最稳的
    ...
    #     if fam_next:
    #         families = fam_next
    #     return model, families, None, None
...
def total():
    ...
    # 2. 配置
    subsample_fractions = [i*30 for i in range(1, 11)]  # 清洗比例
    ...
    for fraction in subsample_fractions:
        ...
        if fraction == 0.0:
            ...
            #
            # Dirty Data + AutoML
            ny_model, families, X_train_grad, X_train_norm = automl(X_train.copy(), y_train, None, 0, fraction)
            ...
            X_train_cleaned = clean(X_train.copy(), train_indices, mask,
                                    'data/' + dataset + '/' + dataset + '_data_vectorized.csv')
            yy_model, families, X_train_grad, X_train_norm = automl(X_train_cleaned.copy(), y_train, None, 0, fraction)
            yy_metric = cal_metric(yy_model, X_test, y_test)
            print("Clean + Auto : ", yy_metric)
            yy_results.append(yy_metric)

            # no loss
            grad_idx, X_train_norm = sample_base_grad(X_train.copy().reset_index(), y_train, clean_fraction/3*2, None, 0)
            mask = np.zeros(len(train_indices), dtype=bool)
            mask[grad_idx] = True
            X_train_cleaned = clean(X_train.copy(), train_indices, mask,
                                    'data/' + dataset + '/' + dataset + '_data_vectorized.csv')
            dh_model, families, X_train_grad, X_train_norm = automl(X_train_cleaned.copy(), y_train, None, 0, int(fraction/3))
            ...
            ng_model, families, X_train_grad, X_train_norm = automl(X_loss, y_loss, None, 0, int(fraction / 3))
            ...
            # Two Stage
            grad_idx, X_train_norm = sample_base_grad(X_train.copy().reset_index(), y_train, clean_fraction / 2, None, 0)
            mask = np.zeros(len(train_indices), dtype=bool)
            mask[grad_idx] = True
            X_train_cleaned = clean(X_train.copy(), train_indices, mask,
                                    'data/' + dataset + '/' + dataset + '_data_vectorized.csv')
            ...
            X_loss = X_train_cleaned.copy().reset_index(drop=True).iloc[loss_idx]
            y_loss = y_train.copy().reset_index(drop=True).iloc[loss_idx]
            two_model, families , X_train_grad, X_train_norm = automl(X_loss, y_loss, None, 0, int(fraction / 2))
            ...
            X_train_norm = None
            Ts = 3
            for t in range(1, Ts):  # 模拟 20 轮自适应选择
                arm = mab.choose()
                ...
                if arm == 'AutoML':  # 'AutoML'
                    ...
                    if flag:
                        ...
                        X_loss = X_train_cleaned.copy().reset_index(drop=True).iloc[loss_idx]
                    y_loss = y_train.copy().reset_index(drop=True).iloc[loss_idx]
                    dmco_model, families, X_train_grad, X_train_norm = automl(X_loss, y_loss, None, 0, int(fraction / Ts))
                    dmco_model.refit(X_train_cleaned, y_train)
                    dmco_metric = cal_metric(dmco_model, X_test, y_test)
                    # ours.append(dmco_metric)
                else:
                    # 清洗：效果提升稍大，但代价偏高
                    print(t, ": clean", clean_fraction)
                    if flag:
                        # first clean
                        grad_idx, X_train_norm = sample_base_grad(X_train.copy().reset_index(), y_train, clean_fraction / Ts,
                                                    X_train_norm, 0)
                        mask = np.zeros(len(train_indices), dtype=bool)
                        mask[grad_idx] = True
                        X_train_cleaned = clean(X_train.copy(), train_indices, mask,
                        ...
                        grad_idx, X_train_norm = sample_base_grad(X_train_cleaned.copy().reset_index(), y_train, clean_fraction / Ts,
                                                    X_train_norm, clean_iter)
                        mask = np.zeros(len(train_indices), dtype=bool)
                        mask[grad_idx] = True
                        X_train_cleaned = clean(X_train_cleaned.copy(), train_indices, mask,
                                                'data/' + dataset + '/' + dataset + '_data_vectorized.csv')
    ...
    # 4. 绘图
    f = open('log/' + dataset + error + '_ori_res_0115.txt', 'w')
    f.write(str(nn_results)+'\n')
    f.write(str(ny_results)+'\n')
    f.write(str(yn_results)+'\n')
    f.write(str(yy_results)+'\n')
    f.write(str(ours)+'\n')
    f.write(str(dh_results)+'\n')
    f.write(str(ng_results)+'\n')
    f.write(str(dh_results)+'\n')
    f.write(str(ours)+'\n')
    f.close()
    ...
    plt.show()
    plt.figure(figsize=(8, 6))
    plt.plot(subsample_fractions, ng_results, marker='o', label='No grad')
    plt.plot(subsample_fractions, dh_results, marker='s', label='No loss')
    plt.plot(subsample_fractions, two_results, marker='^', label='Two Stage')
    plt.plot(subsample_fractions, ours, marker='P', label='DMCO')
    plt.xlabel('Sample fraction (%)')
    plt.ylabel('Test mse')
    plt.title('Effect of sampling for cleaning on SVM Performance')
